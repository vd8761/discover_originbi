"use client";

import React from "react";
import { CheckIcon, AlertCircleIcon } from "@/components/icons";

const WhyOriginBI: React.FC = () => {
    const tableData = [
        {
            feature: "Basis of Choice",
            old: 'Based on "Market Trends" or what relatives/friends suggest.',
            new: "Based on your child’s Unique Behavioral DNA and natural strengths.",
        },
        {
            feature: "The Method",
            old: 'General marks in school or "What do you like?" questions.',
            new: "Proprietary Behavioral Mapping to see how their brain is actually wired.",
        },
        {
            feature: "The Outcome",
            old: "Picking a degree and hoping it leads to a good career.",
            new: "Identifying the Role first, then picking the degree that fits.",
        },
        {
            feature: "Family Stress",
            old: 'High anxiety and "Dinner Table" arguments over college choices.',
            new: "Confidence and Peace of Mind knowing the path is backed by science.",
        },
        {
            feature: "The Goal",
            old: 'Just getting into "A Good College" or any popular branch.',
            new: "Finding the Right Professional Fit for long-term growth and success.",
        },
    ];

    return (
        <section
            id="why-originbi"
            className="relative z-10 w-full py-16 lg:py-24 bg-white dark:bg-brand-dark-primary transition-colors duration-300"
        >
            <div className="max-w-[1920px] mx-auto px-6 lg:px-12 2xl:px-[clamp(24px,8.33vw,160px)]">
                <div className="text-center mb-12 lg:mb-16">
                    <h2 className="text-[clamp(28px,3.4vw,54px)] font-sans font-semibold text-brand-dark-primary dark:text-white leading-tight mb-4 transition-colors duration-300">
                        4: Why OriginBI?
                    </h2>
                    <p className="text-[clamp(18px,1.5vw,24px)] text-gray-600 dark:text-gray-300">
                        The Difference Between Guessing and Knowing
                    </p>
                </div>

                <div className="max-w-6xl mx-auto overflow-hidden rounded-[2rem] border border-gray-200 dark:border-gray-800 shadow-sm bg-white dark:bg-brand-dark-primary transition-colors duration-300">
                    {/* Header Row */}
                    <div className="hidden md:grid grid-cols-[1fr_1.5fr_1.5fr] bg-brand-light-secondary dark:bg-gray-800/50 border-b border-gray-200 dark:border-gray-800">
                        <div className="p-6 lg:p-8 font-semibold text-xl text-brand-dark-primary dark:text-white flex items-center">
                            Feature
                        </div>
                        <div className="p-6 lg:p-8 font-semibold text-xl text-brand-dark-primary dark:text-white flex items-center gap-3">
                            <span className="w-10 h-10 rounded-full bg-red-100 dark:bg-red-500/10 flex items-center justify-center text-red-500 shrink-0">
                                <AlertCircleIcon className="w-5 h-5" />
                            </span>
                            <span className="pt-1">The Old Way <span className="text-gray-500 dark:text-gray-400 text-sm font-medium block mt-0.5">(High Risk)</span></span>
                        </div>
                        <div className="p-6 lg:p-8 font-semibold text-2xl text-brand-green flex items-center gap-3 bg-brand-green/5 dark:bg-brand-green/10 border-l border-brand-green/20">
                            <span className="w-10 h-10 rounded-full bg-brand-green flex items-center justify-center text-white shrink-0 shadow-sm shadow-brand-green/30">
                                <CheckIcon className="w-6 h-6" />
                            </span>
                            <span className="pt-1">The OriginBI Way <span className="text-brand-green/70 text-sm font-medium block mt-0.5">(The Strategy)</span></span>
                        </div>
                    </div>

                    {/* Data Rows */}
                    <div className="divide-y divide-gray-200 dark:divide-gray-800">
                        {tableData.map((row, index) => (
                            <div key={index} className="grid grid-cols-1 md:grid-cols-[1fr_1.5fr_1.5fr] hover:bg-gray-50 dark:hover:bg-gray-800/20 transition-colors duration-300">
                                <div className="p-6 lg:px-8 flex items-center font-medium text-lg text-brand-dark-primary dark:text-gray-200">
                                    <span className="md:hidden font-semibold text-sm text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 block">Feature</span>
                                    {row.feature}
                                </div>

                                <div className="p-6 lg:px-8 flex flex-col justify-center text-gray-600 dark:text-gray-400 text-[1.1rem]">
                                    <span className="md:hidden font-semibold text-sm text-red-500 uppercase tracking-wider mb-3 flex items-center gap-2">
                                        <AlertCircleIcon className="w-4 h-4" /> The Old Way
                                    </span>
                                    <p className="leading-relaxed">{row.old}</p>
                                </div>

                                <div className="p-6 lg:px-8 flex flex-col justify-center text-[1.15rem] text-brand-dark-primary dark:text-white bg-brand-green/[0.03] dark:bg-brand-green/5 md:border-l border-brand-green/20">
                                    <span className="md:hidden font-semibold text-sm text-brand-green uppercase tracking-wider mb-3 flex items-center gap-2">
                                        <CheckIcon className="w-4 h-4" /> The OriginBI Way
                                    </span>
                                    <p className="leading-relaxed font-medium">{row.new}</p>
                                </div>
                            </div>
                        ))}
                    </div>
                </div>

                <div className="mt-16 lg:mt-20 text-center max-w-[50rem] mx-auto">
                    <div className="relative inline-block rounded-3xl bg-brand-green/[0.08] dark:bg-brand-green/[0.12] border border-brand-green/20 px-8 py-10 lg:px-14 lg:py-12 shadow-[0_8px_30px_-12px_rgba(30,211,106,0.2)]">
                        <img
                            src="/quote-symbol.svg"
                            alt=""
                            aria-hidden="true"
                            className="absolute -top-3 -left-3 w-10 h-auto opacity-70 pointer-events-none select-none text-brand-green filter drop-shadow-[0_0_8px_rgba(30,211,106,0.5)]"
                        />
                        <img
                            src="/quote-symbol.svg"
                            alt=""
                            aria-hidden="true"
                            className="absolute -bottom-3 -right-3 w-10 h-auto rotate-180 opacity-70 pointer-events-none select-none filter drop-shadow-[0_0_8px_rgba(30,211,106,0.5)]"
                        />
                        <p className="text-[clamp(20px,1.6vw,30px)] leading-[1.6] text-brand-dark-primary dark:text-white font-medium">
                            <span className="text-gray-600 dark:text-gray-300 font-normal">Most students pick a degree based on 10% of the information (Marks & Trends).</span>
                            <br className="hidden md:block" />
                            <span className="mt-4 block">We help you decide based on the other <strong className="text-brand-green text-[1.2em] font-bold mx-1">90%</strong> <span className="block mt-1 sm:inline sm:mt-0 text-[0.9em] text-brand-dark-primary/80 dark:text-white/80">(Personality, Behavior, and Potential).</span></span>
                        </p>
                    </div>
                </div>
            </div>
        </section>
    );
};

export default WhyOriginBI;
